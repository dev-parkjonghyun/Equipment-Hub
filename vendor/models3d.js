...
// ── seiseX1Reel — SEISE 전선릴 X1 4구 10m → PWR-001/002/003 (색만 다름) ──
//   (출처: PWR-001_SEISE-X1/PWR-001_threejs.js, 자산번호 일치)
const REEL_COLORS = { blue: 0x2f5fbf, red: 0xc22a30, yellow: 0xe0a318 };
function seiseX1Reel(sp) {
    const g = new THREE.Group();
    const R = sp.w / 2, HD = sp.d / 2, yc = R;
    const body = mat(REEL_COLORS[sp.color] || REEL_COLORS.blue, { roughness: 0.55, metalness: 0.05 });
    const black = M.black(), rub = M.rubber(), knobM = M.knob();
    const red = mat(0xcc2222, { roughness: 0.45, metalness: 0.05 });
    const cylZ = (r, len, x, y, z, seg = 16) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [x, y, z], rot: [Math.PI / 2, 0, 0] });
    const box = (w, h, d, x, y, z) => ({ geo: new THREE.BoxGeometry(w, h, d), pos: [x, y, z] });

    const ringShape = new THREE.Shape();
    ringShape.absarc(0, 0, R, 0, Math.PI * 2, false);
    const hole = new THREE.Path(); hole.absarc(0, 0, 0.081, 0, Math.PI * 2, true);
    ringShape.holes.push(hole);
    const front = new THREE.Mesh(new THREE.ExtrudeGeometry(ringShape, { depth: 0.011, bevelEnabled: false, curveSegments: 16 }), body);
    front.position.set(0, yc, HD - 0.011); g.add(front);

    const shell = [];
    shell.push(cylZ(R, 0.011, 0, yc, -(HD - 0.0055)));
    shell.push(cylZ(0.070, sp.d - 0.022, 0, yc, 0));
    const hTop = sp.h, hBase = R * 1.72;
    for (const s of [-1, 1]) shell.push(box(0.014, hTop - hBase, 0.030, s * 0.052, (hTop + hBase) / 2, 0));
    shell.push(box(0.118, 0.016, 0.030, 0, hTop - 0.008, 0));
    g.add(new THREE.Mesh(geoBatch(shell), body));

    const coil = [];
    coil.push(cylZ(0.099, sp.d - 0.026, 0, yc, 0));
    for (const s of [-1, 1]) coil.push(cylZ(0.101, 0.006, 0, yc, s * 0.020));
    g.add(new THREE.Mesh(geoBatch(coil), rub));

    const zF = HD;
    g.add(new THREE.Mesh(geoBatch([cylZ(0.079, 0.008, 0, yc, zF - 0.005)]), black));
    const P = 0.038, sock = [], pin = [];
    for (const [ox, oy] of [[-1, 1], [1, 1], [-1, -1], [1, -1]]) {
        sock.push({ geo: new THREE.CircleGeometry(0.020, 14), pos: [ox * P, yc + oy * P, zF - 0.0008] });
        pin.push({ geo: new THREE.CircleGeometry(0.0075, 10), pos: [ox * P, yc + oy * P, zF - 0.0005] });
    }
    g.add(new THREE.Mesh(geoBatch(sock), knobM));
    g.add(new THREE.Mesh(geoBatch(pin), M.aluDk()));
    const btn = new THREE.Mesh(new THREE.CylinderGeometry(0.008, 0.008, 0.005, 10), red);
    btn.rotation.x = Math.PI / 2; btn.position.set(0, yc - 0.064, zF - 0.0025); g.add(btn);
    const out = [];
    out.push({ geo: new THREE.CylinderGeometry(0.0055, 0.0055, 0.060, 8), pos: [0.070, 0.075, -0.020], rot: [0, 0, Math.PI / 3.4] });
    out.push(box(0.028, 0.038, 0.020, 0.055, 0.048, -0.020));
    g.add(new THREE.Mesh(geoBatch(out), black));
    return g;
}

// ── hollylandMarsM1 — HOLLYLAND Mars M1 5.5" 모니터 → MON-002 ──
//   (출처: MON-002_MarsM1/MON-002_threejs.js, 자산번호 일치)
function marsRoundedRect(hw, hh, r) {
    const s = new THREE.Shape();
    s.moveTo(-hw + r, -hh);
    s.lineTo(hw - r, -hh); s.quadraticCurveTo(hw, -hh, hw, -hh + r);
    s.lineTo(hw, hh - r);  s.quadraticCurveTo(hw, hh, hw - r, hh);
...
function _marsWireless(sp, opt) {   // Mars M1(모니터) · Mars 400s(송수신기) 공용 골격
    const g = new THREE.Group();
    const HW = sp.w / 2, H = sp.h, HD = sp.d / 2;
    const body = M.black(), alu = M.aluDk(), knobM = M.knob();
    const accent = sp.role === 'tx' ? mat(0xc23a30, { roughness: 0.5, metalness: 0.1 }) : mat(0x2f9e5e, { roughness: 0.5, metalness: 0.1 });
    const box = (w, h, d, x, y, z) => ({ geo: new THREE.BoxGeometry(w, h, d), pos: [x, y, z] });
    const cylY = (r, len, x, y, z, seg = 10) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [x, y, z] });
    const cylZ = (r, len, x, y, z, seg = 10) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [x, y, z], rot: [Math.PI / 2, 0, 0] });
    const shell = new THREE.ExtrudeGeometry(marsRoundedRect(HW, H / 2, 0.006), { depth: sp.d - (opt.faceGap || 0), bevelEnabled: false, curveSegments: 2 });
    shell.translate(0, H / 2, -HD); g.add(new THREE.Mesh(shell, body));
    const zF = HD;
    // 전면(스크린 또는 OLED)
    opt.front(g, { HW, H, HD, zF, alu, knobM, accent, box, cylZ, cylY });
    // 후면 NP-F 플레이트
    g.add(new THREE.Mesh(geoBatch([
        box(opt.plateW, opt.plateH, 0.005, 0, H * opt.plateY, -HD + 0.0025),
        box(opt.plateW + 0.008, 0.006, 0.008, 0, H * opt.plateY + opt.plateR, -HD + 0.004),
        box(opt.plateW + 0.008, 0.006, 0.008, 0, H * opt.plateY - opt.plateR, -HD + 0.004),
    ]), alu));
    // 포트 + 1/4" 나사
    const port = [];
    port.push(cylZ(0.0075, 0.006, -opt.bncX, H * 0.13, -HD + 0.003, 10));
    port.push(cylY(0.0055, 0.005, 0, 0.0025, 0, 10));
    g.add(new THREE.Mesh(geoBatch(port), knobM));
    // 안테나 2개
    const a = THREE.MathUtils.degToRad(sp.antAngle || 0);
    for (const s of [-1, 1]) {
        const hinge = new THREE.Group(); hinge.position.set(s * opt.antX, H, 0); hinge.rotation.z = -s * a; g.add(hinge);
        const parts = [];
        parts.push(cylY(0.0065, 0.010, 0, 0.005, 0, 8));
        parts.push(cylY(0.004, sp.antLen || 0.088, 0, 0.010 + (sp.antLen || 0.088) / 2, 0, 8));
        parts.push(cylY(0.0045, 0.006, 0, 0.010 + (sp.antLen || 0.088), 0, 8));
        hinge.add(new THREE.Mesh(geoBatch(parts), body));
    }
    return g;
}
function hollylandMarsM1(sp) {
    return _marsWireless(sp, {
        faceGap: 0.002, plateW: 0.072, plateH: 0.046, plateY: 0.56, plateR: 0.025, bncX: 0.052, antX: 0.056,
        front(g, c) {
            const diag = (sp.screen || 5.5) * 0.0254;
            const sw = diag * 16 / Math.hypot(16, 9), sh = diag * 9 / Math.hypot(16, 9);
            g.add(new THREE.Mesh(geoBatch([c.box(sw + 0.005, sh + 0.005, 0.003, 0, c.H / 2, c.zF - 0.0015)]), c.alu));
            const lcd = new THREE.Mesh(new THREE.PlaneGeometry(sw, sh), mat(0x0d1218, { roughness: 0.07, metalness: 0.05 }));
            lcd.position.set(0, c.H / 2, c.zF - 0.0002); g.add(lcd);
            const acc = new THREE.Mesh(new THREE.BoxGeometry(sw * 0.30, 0.003, 0.003), c.accent);
            acc.position.set(-sw * 0.28, c.H / 2 - sh / 2 - 0.007, c.zF - 0.0015); g.add(acc);
        },
    });
}
// ── hollylandMars400s — HOLLYLAND Mars 400s Pro 무선 송수신기 → ACC-005 ──
function hollylandMars400s(sp) {
    return _marsWireless(sp, {
        faceGap: 0, plateW: 0.070, plateH: 0.044, plateY: 0.52, plateR: 0.024, bncX: 0.038, antX: 0.040,
        front(g, c) {
            g.add(new THREE.Mesh(geoBatch([
                c.box(0.048, 0.026, 0.003, -0.026, c.H * 0.62, c.zF - 0.0025),
                c.box(0.030, 0.008, 0.003, -0.026, c.H * 0.24, c.zF - 0.0025),
            ]), c.alu));
            const oled = new THREE.Mesh(new THREE.PlaneGeometry(0.042, 0.020), mat(0x0c1014, { roughness: 0.08, metalness: 0.05 }));
            oled.position.set(-0.026, c.H * 0.62, c.zF - 0.0005); g.add(oled);
            const acc = new THREE.Mesh(new THREE.BoxGeometry(0.048, 0.004, 0.003), c.accent);
            acc.position.set(-0.026, c.H * 0.40, c.zF - 0.0025); g.add(acc);
        },
    });
}

// ── 그립암 세트 (VALENS VL-40GA 암 + VL-033 그립헤드) → ACC-001/002/003 ──
//   (출처: ACC-001_GripArmSet, 자산번호 일치)
function gripHeadParts() {
    const cylX = (r, len, z = 0) => ({ geo: new THREE.CylinderGeometry(r, r, len, 12), pos: [0, 0, z], rot: [0, 0, Math.PI / 2] });
    const cylY = (r, len, z = 0) => ({ geo: new THREE.CylinderGeometry(r, r, len, 12), pos: [0, 0, z] });
    const cylZ = (r, len, z, seg = 12) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [0, 0, z], rot: [Math.PI / 2, 0, 0] });
    const jaw = [];
    for (const s of [-1, 1]) jaw.push(cylZ(0.029, 0.014, s * 0.008, 14));
    jaw.push(cylY(0.013, 0.086)); jaw.push(cylX(0.013, 0.062));
    const knob = [];
    knob.push(cylZ(0.008, 0.030, 0.030, 10));
    knob.push({ geo: new THREE.CylinderGeometry(0.006, 0.006, 0.060, 8), pos: [0, 0, 0.046], rot: [0, 0, Math.PI / 2] });
    return { jaw, knob };
}
function gripHeadBores(sr) {
    sr = sr || 0.008; const b = [];
    for (const s of [-1, 1]) {
        const y = new THREE.Mesh(new THREE.CircleGeometry(sr, 10)); y.rotation.x = -s * Math.PI / 2; y.position.y = s * 0.0431; b.push(y);
        const x = new THREE.Mesh(new THREE.CircleGeometry(sr, 10)); x.rotation.y = s * Math.PI / 2; x.position.x = s * 0.0311; b.push(x);
    }
    return b;
}
function valensVl40ga(sp) {
    const g = new THREE.Group();
    const chrome = M.chrome(), knobM = M.knob();
    const L = sp.rodLen || 1.016, r = (sp.rodD || 0.016) / 2;
    const rod = new THREE.Mesh(new THREE.CylinderGeometry(r, r, L, 12), chrome);
    rod.rotation.z = Math.PI / 2; rod.position.x = L / 2; g.add(rod);
    const cap = new THREE.Mesh(new THREE.CylinderGeometry(r + 0.0008, r + 0.0008, 0.006, 12), knobM);
    cap.rotation.z = Math.PI / 2; cap.position.x = 0.003; g.add(cap);
    const head = new THREE.Group(); head.position.x = L + 0.028; head.rotation.x = THREE.MathUtils.degToRad(sp.headTilt || 0); g.add(head);
    const { jaw, knob } = gripHeadParts();
    head.add(new THREE.Mesh(geoBatch(jaw), chrome));
    head.add(new THREE.Mesh(geoBatch(knob), knobM));
    for (const b of gripHeadBores()) { b.material = M.aluDk(); head.add(b); }
    const thru = new THREE.Mesh(new THREE.CylinderGeometry(r, r, 0.070, 12), chrome);
    thru.rotation.z = Math.PI / 2; thru.position.x = L - 0.007; g.add(thru);
    return g;
}
function valensVl033(sp) {
    const g = new THREE.Group();
    const bodyM = sp.color === 'black' ? mat(0x25282d, { roughness: 0.55, metalness: 0.55 }) : M.chrome();
    const knobM = M.knob();
    const { jaw, knob } = gripHeadParts();
    g.add(new THREE.Mesh(geoBatch(jaw), bodyM));
    g.add(new THREE.Mesh(geoBatch(knob), knobM));
    for (const b of gripHeadBores((sp.socket || 0.016) / 2)) { b.material = M.aluDk(); g.add(b); }
    return g;
}
// 그립암 세트(자산 배치용): 헤드 + 비스듬히 뻗은 암 (원점 = 헤드 소켓)
function gripArmSet(sp) {
    const g = new THREE.Group();
    g.add(valensVl033({ socket: 0.016 }));
    const arm = valensVl40ga(sp);
    arm.rotation.z = THREE.MathUtils.degToRad(12);
    g.add(arm);
    return g;
}

// 모서리 둥근 사각 단면 (여러 빌더 공용)
...
// ── valensAClamp — VALENS A클램프(스프링/악어 클램프) → ACC-012(9")/013·014(6") ──
function strip(a, b, w, t) {
    const A = new THREE.Vector3(...a), B = new THREE.Vector3(...b);
    const dir = new THREE.Vector3().subVectors(B, A), len = dir.length();
    return { geo: new THREE.BoxGeometry(w, t, len), pos: A.clone().addScaledVector(dir, 0.5).toArray(),
        quat: new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 0, 1), dir.clone().normalize()) };
}
function valensAClamp(sp) {
    const g = new THREE.Group();
    const SIZES = {
        6: { len: 0.152, jaw: 0.051, spread: 0.052, armW: 0.026 },
        9: { len: 0.229, jaw: 0.076, spread: 0.078, armW: 0.032 } };
    const S = SIZES[sp.size] || SIZES[6];
    const AW = S.armW, HAW = AW / 2, op = THREE.MathUtils.clamp(sp.open || 0, 0, 1);
    const armF = S.len * 0.55, armR = S.len * 0.45, gap = S.jaw * op;
    const psi = Math.asin(Math.min((gap / 2) / armF, 0.6)), spread = S.spread, T = 0.0045;
    const steel = mat(0x9aa1aa, { roughness: 0.34, metalness: 0.88 });
    const tipM = sp.tipColor === 'red' ? mat(0x9c2b26, { roughness: 0.88, metalness: 0 }) : M.rubber();
    const gripM = M.rubber(), pinM = M.knob();
    function arm(s) {
        const a = new THREE.Group(); a.position.set(0, 0, -armF); a.rotation.x = -s * psi; g.add(a);
        const H = [0, s * spread / 2, -armR], P = [0, 0, 0], B = [0, s * (T / 2 + 0.002), armF * 0.45], Tp = [0, s * (T / 2), armF];
        a.add(new THREE.Mesh(geoBatch([ strip(H, P, AW, T), strip(P, B, AW, T), strip(B, Tp, AW, T),
            { geo: new THREE.BoxGeometry(AW, 0.014, 0.016), pos: [0, 0, 0] } ]), steel));
        a.add(new THREE.Mesh(geoBatch([ strip([0, s * 0.0055, armF * 0.45], [0, s * 0.0055, armF + 0.002], AW + 0.002, 0.011) ]), tipM));
        a.add(new THREE.Mesh(geoBatch([ strip([0, s * spread / 2 * 0.98, -armR * 0.96], [0, s * spread / 2 * 0.52, -armR * 0.42], AW + 0.004, 0.013) ]), gripM));
    }
    arm(1); arm(-1);
    const pin = new THREE.Mesh(new THREE.CylinderGeometry(0.0035, 0.0035, AW + 0.010, 8), pinM);
    pin.rotation.z = Math.PI / 2; pin.position.set(0, 0, -armF); g.add(pin);
    const coil = [];
    for (const s of [-1, 1]) {
        coil.push({ geo: new THREE.TorusGeometry(0.0090, 0.0016, 4, 10), pos: [s * (HAW + 0.004), 0, -armF], rot: [0, Math.PI / 2, 0] });
        coil.push({ geo: new THREE.TorusGeometry(0.0105, 0.0016, 4, 10), pos: [s * (HAW + 0.0072), 0, -armF], rot: [0, Math.PI / 2, 0] });
    }
    g.add(new THREE.Mesh(geoBatch(coil), steel));
    return g;
}

// ── gentreeECubeVMount — GENTREE E-CUBE V마운트 배터리 → BAT-V-001/002/003 ──
function gentreeECubeVMount(sp) {
    const g = new THREE.Group();
    const CAP = { 290: { d: 0.078 }, 195: { d: 0.059 } };
    const C = CAP[sp.wh] || CAP[290];
    const HW = sp.w / 2, HH = sp.h / 2, D = C.d, PL = 0.012, zBack = -D, zBody = -PL;
    const shellM = mat(0x232629, { roughness: 0.62, metalness: 0.06 }), plateM = M.aluDk(), knobM = M.knob();
    const slot = mat(0x0d0f12, { roughness: 0.9, metalness: 0.05 });
    const ledOn = mat(0x2fd07a, { roughness: 0.3, metalness: 0, emissive: 0x1f8a50, emissiveIntensity: 0.8 });
    const ledOff = mat(0x1a1d21, { roughness: 0.8, metalness: 0.05 });
    const box = (w, h, d, x, y, z) => ({ geo: new THREE.BoxGeometry(w, h, d), pos: [x, y, z] });
    const cylZ = (r, len, x, y, z, seg = 10) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [x, y, z], rot: [Math.PI / 2, 0, 0] });
    const shell = new THREE.ExtrudeGeometry(roundedRect(HW, HH, 0.007), { depth: D - PL, bevelEnabled: false, curveSegments: 2 });
    shell.translate(0, 0, zBack); g.add(new THREE.Mesh(shell, shellM));
    g.add(new THREE.Mesh(geoBatch([
        box(0.052, 0.010, D * 0.55, 0, HH - 0.006, zBack + D * 0.5),
        box(0.001, 0.030, 0.044, HW, -HH * 0.30, zBack + D * 0.55),
        box(0.001, 0.030, 0.044, -HW, -HH * 0.30, zBack + D * 0.55),
        box(0.095, 0.096, PL - 0.002, 0, 0, zBody),
    ]), plateM));
    for (const s of [-1, 1]) {
        const ch = new THREE.Mesh(new THREE.BoxGeometry(0.013, 0.076, 0.011), slot);
        ch.position.set(s * 0.0415, 0, -0.0072); ch.rotation.y = s * 0.18; g.add(ch);
    }
    g.add(new THREE.Mesh(geoBatch([
        box(0.026, 0.014, 0.008, 0, 0.040, -0.006),
        box(0.030, 0.009, 0.005, 0, -0.030, -0.0025),
    ]), knobM));
    const zP = zBack, port = [];
    for (const s of [-1, 1]) port.push(box(0.019, 0.013, 0.006, s * 0.026, -HH * 0.52, zP + 0.003));
    port.push(box(0.011, 0.005, 0.006, 0, -HH * 0.52, zP + 0.003));
    port.push(cylZ(0.0045, 0.005, 0.034, HH * 0.36, zP + 0.0025, 8));
    g.add(new THREE.Mesh(geoBatch(port), knobM));
    g.add(new THREE.Mesh(geoBatch([box(0.062, 0.016, 0.004, -0.008, HH * 0.36, zP + 0.002)]), plateM));
    const on = [], off = [], leds = sp.leds || 10;
    for (let i = 0; i < leds; i++) {
        const seg = box(0.0042, 0.010, 0.003, -0.035 + i * 0.006, HH * 0.36, zP + 0.0015);
        (i < THREE.MathUtils.clamp(sp.level != null ? sp.level : leds, 0, leds) ? on : off).push(seg);
    }
    if (on.length) g.add(new THREE.Mesh(geoBatch(on), ledOn));
    if (off.length) g.add(new THREE.Mesh(geoBatch(off), ledOff));
    return g;
}

// ── nanliteBtBgV — NANLITE BT-BG-V V마운트 배터리 그립 → PWR-005 ──
function nanliteBtBgV(sp) {
    const g = new THREE.Group();
    const HW = sp.w / 2, HD = sp.d / 2, BOSS = 0.018, H = sp.h - BOSS, PL = 0.021, zBack = -HD, zBody = -HD + PL;
    const body = M.black(), alu = M.aluDk(), knobM = M.knob(), bore = mat(0x101216, { roughness: 0.85, metalness: 0.1 });
    const box = (w, h, d, x, y, z) => ({ geo: new THREE.BoxGeometry(w, h, d), pos: [x, y, z] });
    const cylY = (r, len, x, y, z, seg = 12) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [x, y, z] });
    const cylZ = (r, len, x, y, z, seg = 10) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [x, y, z], rot: [Math.PI / 2, 0, 0] });
    const shell = new THREE.ExtrudeGeometry(roundedRect(HW, H / 2, 0.008), { depth: HD - zBody, bevelEnabled: false, curveSegments: 2 });
    shell.translate(0, H / 2, zBody); g.add(new THREE.Mesh(shell, body));
    const yP = H * 0.50;
    g.add(new THREE.Mesh(geoBatch([box(0.086, 0.090, 0.008, 0, yP, zBody - 0.004)]), alu));
    for (const s of [-1, 1]) {
        const rail = new THREE.Mesh(new THREE.BoxGeometry(0.014, sp.railLen || 0.072, 0.017), alu);
        rail.position.set(s * (sp.railGap || 0.086) / 2, yP, zBack + 0.0088); rail.rotation.y = s * 0.22; g.add(rail);
    }
    g.add(new THREE.Mesh(geoBatch([
        box(0.030, 0.018, 0.011, 0, yP + 0.038, zBack + 0.0055),
        cylZ(0.005, 0.012, 0, yP + 0.038, zBack + 0.006, 8),
        box(0.010, 0.034, 0.014, -0.046, yP, zBody - 0.008),
    ]), knobM));
    g.add(new THREE.Mesh(geoBatch([cylY(0.016, BOSS, 0, H + BOSS / 2, 0, 12)]), alu));
    const socket = new THREE.Mesh(new THREE.CircleGeometry(0.0086, 12), bore);
    socket.rotation.x = -Math.PI / 2; socket.position.set(0, sp.h - 0.0004, 0); g.add(socket);
    const kg = new THREE.Group(); kg.position.set(0, H + BOSS / 2, 0); g.add(kg);
    kg.add(new THREE.Mesh(geoBatch([
        { geo: new THREE.CylinderGeometry(0.005, 0.005, 0.026, 8), pos: [0.026, 0, 0], rot: [0, 0, Math.PI / 2] },
        { geo: new THREE.CylinderGeometry(0.010, 0.010, 0.008, 10), pos: [0.041, 0, 0], rot: [0, 0, Math.PI / 2] },
    ]), knobM));
    const bot = new THREE.Mesh(new THREE.CircleGeometry(0.0086, 12), bore);
    bot.rotation.x = Math.PI / 2; bot.position.set(0, 0.0006, 0); g.add(bot);
    const zF = HD, io = [];
    io.push(cylZ(0.0065, 0.010, -0.026, H * 0.16, zF - 0.005, 10));
    io.push(box(0.016, 0.008, 0.004, 0.024, H * 0.16, zF - 0.002));
    io.push(cylZ(0.0028, 0.003, 0.024, H * 0.16 + 0.016, zF - 0.0015, 8));
    io.push(box(0.062, 0.003, 0.003, 0, H * 0.82, zF - 0.0015));
    g.add(new THREE.Mesh(geoBatch(io), knobM));
    return g;
}

// ── rocknrollerR12rt — RockNRoller R12RT 멀티카트 → ETC-001 ──
function rocknrollerR12rt(sp) {
    const g = new THREE.Group();
    const L = THREE.MathUtils.lerp(sp.lenMin || 0.864, sp.lenMax || 1.321, THREE.MathUtils.clamp(sp.ext != null ? sp.ext : 1, 0, 1));
    const HL = L / 2, rR = sp.rearD / 2, fR = sp.frontD / 2, yRail = sp.yRail;
    const frame = M.black(), tyre = M.rubber(), hub = mat(0xe0a318, { roughness: 0.45, metalness: 0.2 }), grip = mat(0x1a1c20, { roughness: 0.92, metalness: 0 });
    const box = (w, h, d, x, y, z) => ({ geo: new THREE.BoxGeometry(w, h, d), pos: [x, y, z] });
    const cylX = (r, len, x, y, z, seg = 16) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [x, y, z], rot: [0, 0, Math.PI / 2] });
    const cylY = (r, len, x, y, z, seg = 10) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [x, y, z] });
    const rx = 0.120, fr = [];
    for (const s of [-1, 1]) fr.push(box(0.024, 0.030, L, s * rx, yRail, 0));
    for (const z of [-HL + 0.12, 0, HL - 0.10]) fr.push(box(rx * 2, 0.022, 0.026, 0, yRail, z));
    for (let i = 0; i < 5; i++) fr.push(box(rx * 2 - 0.010, 0.008, 0.020, 0, yRail + 0.018, -HL + 0.20 + i * (L - 0.34) / 4));
    fr.push(box(0.040, 0.026, 0.070, 0, yRail - 0.028, -HL + 0.10));
    g.add(new THREE.Mesh(geoBatch(fr), frame));
    const zRear = -HL + 0.13, xRear = 0.1325, tyres = [], hubs = [];
    for (const s of [-1, 1]) { tyres.push(cylX(rR, sp.rearW, s * xRear, rR, zRear)); hubs.push(cylX(rR * 0.62, sp.rearW + 0.002, s * xRear, rR, zRear, 12)); }
    const zFront = HL - 0.11, xFront = 0.108, forks = [];
    for (const s of [-1, 1]) {
        tyres.push(cylX(fR, sp.frontW, s * xFront, fR, zFront)); hubs.push(cylX(fR * 0.60, sp.frontW + 0.002, s * xFront, fR, zFront, 12));
        for (const t of [-1, 1]) forks.push(box(0.008, fR + 0.030, 0.030, s * xFront + t * (sp.frontW / 2 + 0.005), fR + (fR + 0.030) / 2 - 0.010, zFront));
        forks.push(cylY(0.016, 0.020, s * xFront, fR * 2 + 0.026, zFront, 10));
        forks.push(box(0.052, 0.010, 0.052, s * xFront, fR * 2 + 0.040, zFront));
        forks.push(box(0.020, yRail - (fR * 2 + 0.040), 0.024, s * xFront, (fR * 2 + 0.045 + yRail) / 2, zFront));
    }
    g.add(new THREE.Mesh(geoBatch(tyres), tyre)); g.add(new THREE.Mesh(geoBatch(hubs), hub)); g.add(new THREE.Mesh(geoBatch(forks), frame));
    const pivotY = sp.pivotY, pivotZ = -HL + 0.06, upH = sp.h - pivotY, post = [];
    for (const s2 of [-1, 1]) post.push(box(0.026, pivotY - yRail, 0.026, s2 * 0.128, (yRail + pivotY) / 2, pivotZ));
    g.add(new THREE.Mesh(geoBatch(post), frame));
    const hg = new THREE.Group(); hg.position.set(0, pivotY, pivotZ); hg.rotation.x = THREE.MathUtils.degToRad(90 * (1 - (sp.handle != null ? sp.handle : 1))); g.add(hg);
    const hx = 0.128, bar = [];
    for (const s of [-1, 1]) bar.push({ geo: new THREE.BoxGeometry(0.026, upH, 0.026), pos: [s * hx, upH / 2, 0] });
    bar.push({ geo: new THREE.BoxGeometry(hx * 2 + 0.026, 0.026, 0.026), pos: [0, upH - 0.013, 0] });
    hg.add(new THREE.Mesh(geoBatch(bar), frame));
    const gp = new THREE.Mesh(new THREE.BoxGeometry(hx * 1.5, 0.026, 0.030), grip); gp.position.set(0, upH - 0.013, 0); hg.add(gp);
    return g;
}

// ── vendictRoverWagon — VENDICT ROVER 캠핑 웨건 → ETC-003 ──
function vendictRoverWagon(sp) {
    const g = new THREE.Group();
    const FABRIC = { black: 0x24262a, ivory: 0xd9cfbd, khaki: 0x6e6a51 };
    const HW = sp.w / 2, R = sp.wheelD / 2, yF = sp.floorY, yR = yF + sp.tubH;
    const HWt = sp.tubW / 2 + 0.008, HDt = sp.tubD / 2 + 0.008;
    const steel = M.black(), tyreM = M.rubber(), hubM = mat(0x4a4f56, { roughness: 0.5, metalness: 0.45 }), knobM = M.knob();
    const cloth = mat(FABRIC[sp.color] || FABRIC.black, { roughness: 0.94, metalness: 0 });
    const box = (w, h, d, x, y, z) => ({ geo: new THREE.BoxGeometry(w, h, d), pos: [x, y, z] });
    const tubeX = (r, len, x, y, z, seg = 6) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [x, y, z], rot: [0, 0, Math.PI / 2] });
    const tubeY = (r, len, x, y, z, seg = 6) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [x, y, z] });
    const tubeZ = (r, len, x, y, z, seg = 6) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [x, y, z], rot: [Math.PI / 2, 0, 0] });
    const T = 0.011, xRail = 0.150, yRail = 0.145, zF = 0.320, zR = -0.345, xPost = HWt, zPost = HDt, fr = [];
    for (const s of [-1, 1]) fr.push(tubeZ(T, sp.d - 0.06, s * xRail, yRail, 0, 6));
    for (const z of [zF, 0, zR]) fr.push(tubeX(T, xRail * 2, 0, yRail, z, 6));
    for (const z of [zF, zR]) for (const s of [-1, 1]) fr.push(tubeX(0.008, 0.12, s * (xRail + 0.05), R, z, 6));
    for (const z of [zF, zR]) for (const s of [-1, 1]) fr.push(rod([s * xRail, yRail, z], [s * (xRail + 0.075), R, z], 0.009, 6));
    for (const s of [-1, 1]) for (const z of [-1, 1]) fr.push(tubeY(T, yR - yRail, s * xPost, (yRail + yR) / 2, z * zPost, 6));
    for (const s of [-1, 1]) fr.push(tubeZ(T, zPost * 2, s * xPost, yR, 0, 6));
    for (const z of [-1, 1]) fr.push(tubeX(T, xPost * 2, 0, yR, z * zPost, 6));
    for (const s of [-1, 1]) {
        fr.push(rod([s * (xPost + 0.010), yRail + 0.01, -zPost + 0.02], [s * (xPost + 0.010), yR - 0.01, zPost - 0.02], 0.007, 6));
        fr.push(rod([s * (xPost + 0.010), yR - 0.01, -zPost + 0.02], [s * (xPost + 0.010), yRail + 0.01, zPost - 0.02], 0.007, 6));
    }
    for (const s of [-1, 1]) fr.push(rod([s * 0.115, yR, -zPost], [s * 0.115, yR + 0.085, -zPost - 0.045], 0.009, 6));
    fr.push({ geo: new THREE.CylinderGeometry(0.009, 0.009, 0.230, 6), pos: [0, yR + 0.085, -zPost - 0.045], rot: [0, 0, Math.PI / 2] });
    g.add(new THREE.Mesh(geoBatch(fr), steel));
    const tyres = [], hubs = [], xW = HW - sp.wheelW / 2;
    for (const z of [zF, zR]) for (const s of [-1, 1]) { tyres.push(tubeX(R, sp.wheelW, s * xW, R, z, 12)); hubs.push(tubeX(R * 0.52, sp.wheelW + 0.002, s * xW, R, z, 8)); }
    g.add(new THREE.Mesh(geoBatch(tyres), tyreM)); g.add(new THREE.Mesh(geoBatch(hubs), hubM));
    const W = 0.008, tub = [];
    tub.push(box(HWt * 2, W, HDt * 2, 0, yF, 0));
    for (const s of [-1, 1]) tub.push(box(W, sp.tubH, HDt * 2, s * HWt, yF + sp.tubH / 2, 0));
    tub.push(box(HWt * 2, sp.tubH, W, 0, yF + sp.tubH / 2, HDt));
    tub.push(box(HWt * 2, sp.tubH, W, 0, yF + sp.tubH / 2, -HDt));   // 뒷면(닫힘)
    g.add(new THREE.Mesh(geoBatch(tub), cloth));
    // 신축 손잡이 (수직)
    const hlen = sp.handleLen || 0.35, hh = new THREE.Group();
    hh.position.set(0, yR, zPost); hh.rotation.x = -THREE.MathUtils.degToRad(sp.handleDeg != null ? sp.handleDeg : 90); g.add(hh);
    hh.add(new THREE.Mesh(geoBatch([
        { geo: new THREE.CylinderGeometry(0.010, 0.010, hlen, 6), pos: [-0.11, hlen / 2, 0] },
        { geo: new THREE.CylinderGeometry(0.010, 0.010, hlen, 6), pos: [0.11, hlen / 2, 0] },
        { geo: new THREE.CylinderGeometry(0.011, 0.011, 0.24, 6), pos: [0, hlen, 0], rot: [0, 0, Math.PI / 2] },
    ]), steel));
    return g;
}

// ── handTruck2in1 — 2in1 접이식 구르마(핸드트럭↔플랫폼) → ETC-002 ──
function handTruck2in1(sp) {
    const g = new THREE.Group();
    const isPlat = sp.mode === 'platform';
    const R = sp.rearD / 2, cR = sp.casterD / 2;
    const xRail = sp.frameW / 2 - 0.014, xW = sp.w / 2 - sp.rearW / 2;
    const alu = M.alu(), aluDk = M.aluDk(), tyre = M.rubber();
    const hub = mat(0x3a3f46, { roughness: 0.5, metalness: 0.45 });
    const grip = mat(0x1a1c20, { roughness: 0.92, metalness: 0 });
    const deckM = mat(0x8d949c, { roughness: 0.42, metalness: 0.80 });
    const box = (w, h, d, x, y, z) => ({ geo: new THREE.BoxGeometry(w, h, d), pos: [x, y, z] });
    const cylX = (r, len, x, y, z, seg = 12) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [x, y, z], rot: [0, 0, Math.PI / 2] });
    const cylY = (r, len, x, y, z, seg = 8) => ({ geo: new THREE.CylinderGeometry(r, r, len, seg), pos: [x, y, z] });
    const tyres = [], hubs = [];
    for (const s of [-1, 1]) { tyres.push(cylX(R, sp.rearW, s * xW, R, 0, 12)); hubs.push(cylX(R * 0.46, sp.rearW + 0.002, s * xW, R, 0, 10)); }
    g.add(new THREE.Mesh(geoBatch(tyres), tyre)); g.add(new THREE.Mesh(geoBatch(hubs), hub));
    g.add(new THREE.Mesh(geoBatch([cylX(0.010, sp.w - 0.06, 0, R, 0, 8)]), aluDk));
    const fr = new THREE.Group();
    if (isPlat) { fr.position.set(0, sp.deckY, 0.100); fr.rotation.x = -Math.PI / 2; } else { fr.position.set(0, 0.020, -0.020); }
    g.add(fr);
    const L = sp.lowerLen, rail = [];
    for (const s of [-1, 1]) rail.push(box(0.028, L, 0.024, s * xRail, L / 2, 0));
    for (const y of [0.06, L * 0.42, L - 0.03]) rail.push(box(xRail * 2, 0.022, 0.020, 0, y, 0));
    rail.push(box(xRail * 2 + 0.028, L - 0.04, 0.006, 0, L / 2, 0.015));
    fr.add(new THREE.Mesh(geoBatch(rail), alu));
    fr.add(new THREE.Mesh(geoBatch([box(0.030, 0.060, 0.040, -xRail, 0.030, -0.024), box(0.030, 0.060, 0.040, xRail, 0.030, -0.024)]), aluDk));
    const hg = new THREE.Group(); hg.position.set(0, L, 0); hg.rotation.x = isPlat ? Math.PI / 2 : 0; fr.add(hg);
    const HL = sp.handleLen, hb = [];
    for (const s of [-1, 1]) hb.push(box(0.026, HL, 0.022, s * xRail, HL / 2, 0));
    hb.push(box(xRail * 2 + 0.026, 0.026, 0.022, 0, HL - 0.013, 0));
    hb.push(box(xRail * 2, 0.020, 0.018, 0, HL * 0.45, 0));
    hg.add(new THREE.Mesh(geoBatch(hb), alu));
    const gp = new THREE.Mesh(new THREE.BoxGeometry(xRail * 1.5, 0.028, 0.026), grip); gp.position.set(0, HL - 0.013, 0); hg.add(gp);
    const ng = new THREE.Group(); ng.position.set(0, 0.014, 0.014); ng.rotation.x = isPlat ? -Math.PI / 2 : 0; fr.add(ng);
    ng.add(new THREE.Mesh(geoBatch([
        box(sp.noseW, 0.010, sp.noseD, 0, 0, sp.noseD / 2 + 0.004),
        box(sp.noseW - 0.06, 0.014, 0.018, 0, 0.008, sp.noseD - 0.010),
    ]), deckM));
    if (isPlat) {
        const zC = 0.100 - L + 0.090, legs = [], ct = [], ch = [];
        for (const s of [-1, 1]) {
            legs.push(box(0.024, sp.deckY - cR * 2 - 0.010, 0.024, s * xRail, (sp.deckY + cR * 2 - 0.010) / 2, zC));
            legs.push(box(0.052, 0.012, 0.052, s * xRail, cR * 2 + 0.006, zC));
            legs.push(cylY(0.012, 0.016, s * xRail, cR * 2 - 0.004, zC, 8));
            ct.push(cylX(cR, 0.030, s * xRail, cR, zC, 10)); ch.push(cylX(cR * 0.45, 0.032, s * xRail, cR, zC, 8));
        }
        g.add(new THREE.Mesh(geoBatch(legs), aluDk)); g.add(new THREE.Mesh(geoBatch(ct), tyre)); g.add(new THREE.Mesh(geoBatch(ch), hub));
    } else {
        const st = [], ct = [];
        for (const s of [-1, 1]) { st.push(box(0.022, 0.120, 0.022, s * xRail, L * 0.62, -0.030)); ct.push(cylX(cR, 0.030, s * xRail, L * 0.62 - 0.076, -0.036, 10)); }
        fr.add(new THREE.Mesh(geoBatch(st), aluDk)); fr.add(new THREE.Mesh(geoBatch(ct), tyre));
    }
    return g;
}
...
// SEISE 전선릴 X1 4구 10m → PWR-001/002/003 (같은 제품, 색만 구분)
[['PWR-001', 'blue'], ['PWR-002', 'red'], ['PWR-003', 'yellow']].forEach(([id, color]) => {
    SPECS[id] = { w: 0.218, h: 0.275, d: 0.088, color, outlets: 4, src: 'est' };
});
// HOLLYLAND Mars M1 5.5" 모니터 → MON-002
SPECS['MON-002'] = { w: 0.152, h: 0.096, d: 0.040, screen: 5.5, role: 'rx', antAngle: 0, antLen: 0.088, src: 'est' };
// HOLLYLAND Mars 400s Pro 무선 송수신기 → ACC-005
SPECS['ACC-005'] = { w: 0.112, h: 0.072, d: 0.034, role: 'tx', antAngle: 0, antLen: 0.088, src: 'est' };
// VALENS VL-40GA 그립암 + VL-033 그립헤드 세트 → ACC-001/002/003
['ACC-001', 'ACC-002', 'ACC-003'].forEach(id => {
    SPECS[id] = { w: 1.075, h: 0.095, d: 0.052, rodLen: 1.016, rodD: 0.016, headTilt: 0, src: 'est' };
});
// VALENS A클램프 — 9인치(ACC-012) · 6인치(ACC-013/014)
SPECS['ACC-012'] = { w: 0.042, h: 0.085, d: 0.229, size: 9, open: 0, tipColor: 'black', src: 'est' };
['ACC-013', 'ACC-014'].forEach(id => {
    SPECS[id] = { w: 0.036, h: 0.057, d: 0.152, size: 6, open: 0, tipColor: 'black', src: 'est' };
});
// GENTREE E-CUBE V마운트 배터리 → BAT-V-001/002/003 (같은 제품)
['BAT-V-001', 'BAT-V-002', 'BAT-V-003'].forEach(id => {
    SPECS[id] = { w: 0.097, h: 0.146, d: 0.078, wh: 290, leds: 10, level: 10, src: 'est' };
});
// NANLITE BT-BG-V V마운트 배터리 그립 → PWR-005
SPECS['PWR-005'] = { w: 0.1020, h: 0.1565, d: 0.0935, railGap: 0.086, railLen: 0.072, knobDeg: 0, src: 'est' };
// RockNRoller R12RT 멀티카트 → ETC-001
SPECS['ETC-001'] = { w: 0.343, h: 1.054, d: 1.321, ext: 1, handle: 1, lenMin: 0.864, lenMax: 1.321,
    rearD: 0.254, rearW: 0.076, frontD: 0.203, frontW: 0.051, yRail: 0.165, pivotY: 0.290, src: 'est' };
// 2in1 접이식 구르마(핸드트럭↔플랫폼) → ETC-002
SPECS['ETC-002'] = { w: 0.500, h: 1.250, d: 0.550, mode: 'upright', rearD: 0.254, rearW: 0.050,
    casterD: 0.100, frameW: 0.400, lowerLen: 0.760, handleLen: 0.470, noseW: 0.500, noseD: 0.280, deckY: 0.285, src: 'est' };
...
function isCableReel(eq) { return eq && (['PWR-001','PWR-002','PWR-003'].includes(eq.id) || /전선릴|전기\s*릴|릴선|reel|seise/i.test(eq.product || '')); }
// HOLLYLAND Mars M1 모니터 → MON-002
function isMarsM1(eq) { return eq && (eq.id === 'MON-002' || /mars\s*m1/i.test(eq.product || '')); }
// HOLLYLAND Mars 400s Pro 송수신기 → ACC-005
function isMars400(eq) { return eq && (eq.id === 'ACC-005' || /mars\s*400/i.test(eq.product || '')); }
// VALENS 그립암/그립헤드 세트 → ACC-001/002/003
function isGripArmSet(eq) { return eq && (['ACC-001','ACC-002','ACC-003'].includes(eq.id) || /그립\s*암|그립\s*헤드|griparm|grip\s*head|vl-?40ga|vl-?033/i.test(eq.product || '')); }
// VALENS A클램프 → ACC-012/013/014
function isAClamp(eq) { return eq && (['ACC-012','ACC-013','ACC-014'].includes(eq.id) || /a\s*클램프|클램프|clamp|vl-?0[69]c/i.test(eq.product || '')); }
// GENTREE E-CUBE V마운트 배터리 → BAT-V-001/002/003 (FXLION BAT-V-004 제외)
function isECubeBat(eq) { return eq && (['BAT-V-001','BAT-V-002','BAT-V-003'].includes(eq.id) || /e-?cube|gentree|젠트리|gentree/i.test(eq.product || '')); }
// NANLITE BT-BG-V 배터리 그립 → PWR-005 (BH-FZ260-V PWR-004 와 구분)
function isBtBgV(eq) { return eq && (eq.id === 'PWR-005' || /bt-?bg-?v/i.test(eq.product || '')); }
// RockNRoller 멀티카트 → ETC-001
