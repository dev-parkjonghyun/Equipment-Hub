...
#app.view-only{padding-top:38px;height:100vh}
#app.view-only #rail .rail-btn.tool[data-p=scenes],
#app.view-only #list-tools,
#app.view-only #layout-tools .seg,
#app.view-only #panel-tab{display:none}
#app.view-only .eq-card{cursor:default;opacity:.85}
#app.view-only #toolbar button:not(.md):not(#panel-tab){opacity:.45;pointer-events:none}
...
<div id="app">
    ...
    <div id="sb-modal" class="modal">
      <div class="mbox">
        <div class="mhead">서버 연결 설정</div>
        <p class="msub">공유 링크를 쓰려면 Supabase 주소와 키가 필요합니다.<br>
          <b>주소</b>는 Settings → Data API 에서, <b>키</b>는 Settings → API Keys 에서 복사하세요.<br>
          <b>Publishable key 는 공개돼도 되는 값</b>입니다.
          Secret key(sb_secret_…)는 절대 넣지 마세요.</p>
        <label class="mlab">Project URL</label>
        <input id="sb-url" placeholder="https://abcdefgh.supabase.co">
        <label class="mlab">Publishable key (예전 이름: anon public)</label>
        <input id="sb-anon" placeholder="sb_publishable_… 또는 eyJ…">
        <div class="mbtns">
          <button onclick="closeShareSetup()">취소</button>
          <button class="primary" onclick="saveShareSetup()">저장</button></div>
      </div>
    </div>
    ...
    <main id="main">
        <div id="toolbar">
            ...
            <span id="three-tools" style="display:none">
                ...
                <label class="tlab">초점 <input id="foc-in" type="number" step="1" min="8" max="400"
                    value="35" onchange="setFocal(this.value)"> mm</label>
            ...
            <div id="cam-panel">
                ...
                <div id="cp-body">
                    ...
                    <div class="cp-btns">
                        ...
                        <button onclick="cyclePreviewSize()">⤢ 크기</button>
                        <button onclick="toggleGuides()">⊞ 가이드</button>
                    </div>
...
function sceneForShare() {
    ...
    return {
        ...
        floor: sc.floor,
        sets: state.sets,
        eqEdits: state.eqEdits || {},
        madeAt: new Date().toISOString(),
    };
}
const SHARE_DAYS = [7, 30, 90];
async function createShareLink() {
    if (!sbReady()) { openShareSetup(); return; }
    const sc = currentScene();
...
function openShareSetup() {
    ...
    document.getElementById('sb-anon').value = c.anon;
    document.getElementById('sb-modal').classList.add('on');
}
function closeShareSetup() { document.getElementById('sb-modal').classList.remove('on'); }
function saveShareSetup() {
    ...
    if (anon) {
        if (/^sb_secret_/.test(anon) || /service_role/.test(anon)) {
            alert('비밀 키(Secret key)를 넣으셨습니다.\n\n'
                + '이 키는 접근 제한을 무시하므로 웹에 올리면 안 됩니다.\n'
                + 'Publishable key (sb_publishable_...) 를 사용해 주세요.');
            return;
        }
        const okNew = /^sb_publishable_/.test(anon);
        const okOld = /^eyJ/.test(anon) && anon.length > 60;
        if (!okNew && !okOld) {
            alert('키 형식이 올바르지 않습니다.\n\n'
                + '· 새 형식: sb_publishable_ 로 시작\n'
                + '· 옛 형식: eyJ 로 시작하는 긴 문자열');
            return;
        }
    }
    state.sb = { url, anon };
    saveState(); closeShareSetup(); renderScenePane();
    syncAuthUI();
    if (url) loadEquipmentFromServer().then(() => { renderList(); renderPalette(); });
    setStatus(url ? '서버 설정을 저장했습니다' : '서버 설정을 비웠습니다');
}
...
async function doLogin() {
    ...
    try {
        ...
        syncAuthUI();
        await loadEquipmentFromServer();
        renderList(); renderPalette(); renderScenePane();
...
function syncAuthUI() {
    ...
    if (isLoggedIn()) {
        ...
        b.title = sbReady() ? '스튜디오 계정으로 로그인' : '먼저 서버를 연결해 주세요';
        b.onclick = openLogin;
    }
    b.style.display = sbReady() ? 'inline-flex' : 'none';
}
// 수정하려 할 때 로그인 확인
function requireLogin(what) {
    if (!sbReady()) return true;              // 서버 연결 전이면 이 브라우저에만 저장 (기존 동작)
...
function pickGearPhoto() {
    if (!sbReady()) { alert('먼저 서버를 연결해 주세요.\n좌측 씬 패널 → 서버 연결하기'); openShareSetup(); return; }
...
function updateListSummary() {
    ...
    el.innerHTML = cards.map(c =>
        ...
        + `<div class="lsum src ${src.c}" onclick="${eqIsServer() && !isLoggedIn() ? 'openLogin()' : 'openShareSetup()'}">
...
function renderScenePane() {
    ...
    h += `<div class="pane-btns">
          ...
          <div class="slot-help" style="margin-bottom:8px">
            ${sbReady()
              ? '링크를 만들면 클라이언트·협력사가 <b>보기만</b> 할 수 있습니다.<br>언제든 회수할 수 있어요.'
              : '<b>서버 연결이 아직 없습니다.</b><br>설정하면 짧은 링크로 배치를 공유할 수 있어요.'}
          </div>
          <div class="pane-btns">
            ${sbReady()
              ? `<button class="primary" onclick="createShareLink()">🔗 공유 링크 만들기</button>
                 <button onclick="openShareSetup()">⚙ 서버</button>`
              : `<button class="primary" onclick="openShareSetup()">⚙ 서버 연결하기</button>`}
          </div>
          ...
          <div class="pane-sec">백업 · 공유</div>
          <div class="slot-help" style="margin-bottom:8px">
            작업 내용은 이 브라우저에만 저장됩니다.<br>
            <b>다른 컴퓨터로 옮기거나</b> 팀원에게 보낼 때, 또는
            <b>실수로 지워질 경우에 대비</b>해 파일로 받아두세요.
          </div>
          <div class="pane-btns">
            <button onclick="exportJSON()">📥 파일로 저장</button>
            <button onclick="importJSON()">📤 파일 불러오기</button></div>`;
    box.innerHTML = h;
}
...
function build3D() {
    ...
    if (n) { R3.orbit.tx = cx / n; R3.orbit.tz = cz / n; R3.orbit.ty = f.ceilH * 0.45; }
...
function setFocal(v) {
    ...
    document.getElementById('foc-in').value = focalMM;
    build3D();
}
function pickLens(v) {
    if (!v) return;
