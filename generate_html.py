...
#floor-svg{display:block;background:#0f141a;cursor:default;touch-action:none}
...
/* ═══ 차량 적재 체크리스트 (전체 화면) ═══ */
#checklist-view{position:fixed;inset:0;z-index:500;background:var(--bg-0);
  display:flex;flex-direction:column}
#cl-bar{display:flex;align-items:center;gap:14px;flex-wrap:wrap;
  padding:14px 20px;background:var(--bg-1);border-bottom:1px solid var(--line);
  flex:0 0 auto}
#cl-title{flex:1;min-width:160px}
#cl-scene{font-size:17px;font-weight:700;color:var(--tx-0);letter-spacing:-0.01em}
#cl-prog{font-size:13px;color:var(--tx-2);margin-top:3px}
#cl-prog b{color:var(--ok);font-weight:700}
#cl-actions{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
#cl-onlyleft{display:inline-flex;align-items:center;gap:7px;font-size:13px;
  color:var(--tx-1);cursor:pointer;user-select:none;
  padding:8px 12px;background:var(--bg-2);border:1px solid var(--line-2);border-radius:var(--r-m)}
#cl-onlyleft input{width:18px;height:18px;accent-color:var(--acc);cursor:pointer}
#cl-actions button{min-height:44px;padding:0 18px;font-size:13.5px}
#cl-progbar{height:5px;background:var(--bg-2);flex:0 0 auto}
#cl-progfill{height:100%;width:0;background:var(--ok);transition:width .2s}
#cl-body{flex:1;overflow-y:auto;-webkit-overflow-scrolling:touch;
  padding:8px 16px 60px;max-width:820px;width:100%;margin:0 auto}
.cl-cat{margin-top:20px}
.cl-cat-h{display:flex;align-items:center;gap:9px;font-size:13px;font-weight:700;
  color:var(--tx-2);text-transform:none;letter-spacing:.02em;
  padding:6px 4px;position:sticky;top:0;background:var(--bg-0);z-index:1}
.cl-cat-h .cl-cat-n{color:var(--tx-3);font-weight:600;font-size:12px}
.cl-row{display:flex;align-items:center;gap:14px;min-height:56px;
  padding:10px 14px;margin:8px 0;background:var(--bg-2);border:1px solid var(--line);
  border-radius:var(--r-l);cursor:pointer;transition:background .1s,border-color .1s;
  user-select:none;-webkit-tap-highlight-color:transparent}
.cl-row:hover{background:var(--bg-3)}
.cl-row:active{background:var(--bg-4)}
.cl-box{width:30px;height:30px;flex:0 0 auto;border-radius:8px;
  border:2px solid var(--line-2);background:var(--bg-0);
  display:flex;align-items:center;justify-content:center;
  font-size:19px;color:#fff;transition:background .1s,border-color .1s}
.cl-tx{flex:1;min-width:0}
.cl-name{font-size:15.5px;font-weight:600;color:var(--tx-0);line-height:1.35}
.cl-id{font-size:11.5px;color:var(--tx-3);font-family:ui-monospace,Menlo,monospace;margin-top:2px}
.cl-part-tag{display:inline-block;font-size:10px;color:var(--tx-2);
  background:var(--bg-4);border-radius:4px;padding:1px 6px;margin-left:7px;
  font-weight:600;vertical-align:middle}
.cl-row.done{border-color:var(--acc-dim)}
.cl-row.done .cl-box{background:var(--ok);border-color:var(--ok);color:var(--bg-0)}
.cl-row.done .cl-name{color:var(--tx-3);text-decoration:line-through}
.cl-row.done .cl-id{color:var(--tx-3)}
.cl-empty{text-align:center;color:var(--tx-3);font-size:13px;padding:60px 20px;line-height:1.7}
@media(max-width:560px){
  #cl-bar{padding:12px 14px}
  #cl-scene{font-size:15px}
  #cl-body{padding:8px 10px 60px}
}
...
#share-btn{background:var(--bg-2);border:1px solid var(--line-2);color:var(--tx-1);
  padding:5px 11px;font-size:11.5px;border-radius:var(--r-m);white-space:nowrap;box-shadow:none;flex:0 0 auto}
#share-btn:hover{border-color:var(--acc);color:var(--tx-0)}
#share-btn.on{background:var(--acc-soft);border-color:var(--acc);color:var(--acc)}
...
#nav-scrim{display:none;position:fixed;inset:0;background:rgba(4,7,11,.55);z-index:190;
  backdrop-filter:blur(1px)}

/* ═══ 모바일 최적화 (스마트폰, ~720px) ═══ */
@media (max-width:720px){
  /* 레일+패널을 오프캔버스 서랍으로. 메인이 전체 폭 */
  #app, #app.pal-hidden{grid-template-columns:1fr}
  #main{grid-column:1 / -1;min-width:0}
  #rail, #panel{position:fixed;top:0;height:100dvh;z-index:200;
    transition:transform .22s ease}
  #rail{left:0;width:78px;transform:translateX(-100%);
    padding-bottom:calc(92px + env(safe-area-inset-bottom))}
  #panel{left:78px;width:min(262px, calc(86vw - 78px));
    transform:translateX(calc(-100% - 78px))}
  /* pal-hidden(데스크톱 접기)이 켜져 있어도 모바일에선 서랍이 대체 */
  #app.pal-hidden #panel{width:min(262px, calc(86vw - 78px));
    visibility:visible;border-right:1px solid var(--line);padding:0}
  #app.nav-open #rail, #app.nav-open #panel{transform:none}
  ...
  #scene-select{min-width:130px}
  #lq{width:150px !important}
  #toolbar button{min-height:40px}
  /* 입력칸: 16px (탭 시 자동 확대 방지 보강) */
  ...
  /* 터치 타깃 넉넉히 */
  .eq-card{min-height:46px;padding:9px 11px}
  .plist-item{min-height:46px}
  .rail-btn{padding:13px 2px 11px}

  /* 모달: 세로로 길면 스크롤 */
  .mbox{max-height:88dvh;overflow:auto}
  .mbox.wide{width:min(760px,94vw)}
  .ph-wrap{grid-template-columns:1fr !important}
}
</style>
</head>
<body>
<div id="app">
    ...
    <div id="share-modal" class="modal">
      <div class="mbox">
        <div class="mhead">🔗 이 씬의 공유 링크</div>
        <p class="msub">받은 사람은 <b>보기만</b> 할 수 있습니다. 씬마다 링크는 <b>하나</b>예요.<br>
          ✏️ 배치도를 수정하면 이 링크에 <b>자동 반영</b>됩니다 (상대방은 새로고침 시 최신).</p>
        ...
        <div class="mbtns">
          <button id="share-revoke" class="danger" style="display:none">링크 회수</button>
          <button onclick="closeShare()">닫기</button></div>
      </div>
    </div>

    <!-- 차량 적재 체크리스트 (전체 화면 오버레이) -->
    <div id="checklist-view" style="display:none">
      <div id="cl-bar">
        <div id="cl-title">
          <div id="cl-scene">차량 체크리스트</div>
          <div id="cl-prog">실음 0 / 0</div>
        </div>
        <div id="cl-actions">
          <label id="cl-onlyleft"><input type="checkbox" id="cl-only" onchange="renderChecklist()"> 안 실은 것만</label>
          <button onclick="resetChecklist()">전체 해제</button>
          <button class="primary" onclick="closeChecklist()">닫기</button>
        </div>
      </div>
      <div id="cl-progbar"><div id="cl-progfill"></div></div>
      <div id="cl-body"></div>
    </div>
    ...
    <main id="main">
        <div id="toolbar">
            <button id="nav-ham" onclick="toggleNav()" title="메뉴" aria-label="메뉴">☰</button>
            <select id="scene-select" style="display:none"></select>
            <span id="scene-chip"></span>
            <button id="auth-chip" style="display:none"></button>
            <button id="share-btn" onclick="shareCurrentScene()"
                    title="이 배치를 보기 전용 링크로 공유합니다 (씬당 1개, 수정 자동 반영)">🔗 공유</button>
...
function saveState() {
    ...
    if (typeof isLoggedIn === 'function' && isLoggedIn()) {
        ...
        // 현재 씬에 공유 링크가 있으면 그 링크도 최신 배치로 자동 갱신
        const cur = currentScene();
        if (cur && cur.shareId) scheduleShareSync(state.currentScene);
    }
}
...
function updateSceneChip() {
    ...
    if (sb) {
        const shared = !!currentScene().shareId;
        sb.classList.toggle('on', shared);
        sb.textContent = shared ? '🔗 공유 중' : '🔗 공유';
    }
}
function switchMode(m) {
    ...
    if (isMobile()) closeNav();   // 모드를 고르면 서랍을 닫아 캔버스를 보여준다
}
...
function shareUrlOf(id) { return location.origin + location.pathname + '?s=' + id; }
// 씬마다 공유 링크 1개. 이미 있으면 그 링크를 재사용(같은 행을 갱신)한다.
async function shareCurrentScene() {
    if (!requireLogin('공유 링크')) return;
    const sc = currentScene();
    const days = SHARE_DAYS[Math.max(0, SHARE_DAYS.indexOf(+(sc.shareDays || state.shareDays || 30)))] || 30;
    const id = sc.shareId || shareId();
    const reuse = !!sc.shareId;
    const payload = sceneForShare(sc);
    const bytes = new Blob([JSON.stringify(payload)]).size;
    if (bytes > 2.8 * 1024 * 1024) {
        alert(`씬이 너무 큽니다 (${(bytes / 1024 / 1024).toFixed(1)}MB).\n배경 도면을 지우거나 더 작은 사진으로 바꿔주세요.`);
        return;
    }
    setStatus(reuse ? '공유 링크 갱신 중…' : '공유 링크 만드는 중…');
    try {
        if (!(await ensureAuth())) throw new Error('로그인이 만료되었습니다. 다시 로그인해 주세요.');
        // upsert — 같은 id 면 갱신, 없으면 생성 → 씬당 행 1개 유지 + 만료 갱신
    ...
    sc.shareId = id; sc.shareAt = Date.now(); sc.shareDays = days;
    saveState();
    showShareResult(shareUrlOf(id), days, bytes, id);
    updateSceneChip();
    renderScenePane();
}
// 이전 이름 호환
function createShareLink() { return shareCurrentScene(); }
// 날짜+시간 표기 (예: "8/12 오후 2:30")
function fmtShareTime(t) {
    return new Date(t).toLocaleString('ko-KR',
        { month: 'numeric', day: 'numeric', hour: 'numeric', minute: '2-digit' });
}
function showShareResult(link, days, bytes, id) {
    ...
    const rv = document.getElementById('share-revoke');
    if (rv) { rv.style.display = id ? 'inline-flex' : 'none'; rv.onclick = id ? () => revokeShare(id) : null; }
...
async function revokeShare(id) {
    ...
    // 이 링크를 쓰던 씬에서 shareId 제거 → 다음에 다시 누르면 새 링크가 만들어진다
    for (const s of Object.values(state.scenes)) if (s.shareId === id) { delete s.shareId; delete s.shareAt; delete s.shareDays; }
    state.shares = (state.shares || []).filter(x => x.id !== id);   // 옛 목록 정리
    closeShare();
    saveState(); renderScenePane(); updateSceneChip();
...
// 공유 링크가 있는 씬을 수정하면, 그 링크(gear_scenes)를 최신 배치로 자동 갱신한다.
let _shareTimer = null;
function scheduleShareSync(key) {
    clearTimeout(_shareTimer);
    _shareTimer = setTimeout(() => { pushSharedScene(key); }, 2000);
}
async function pushSharedScene(key) {
    if (!isLoggedIn() || viewOnly) return;
    const sc = state.scenes[key];
    if (!sc || !sc.shareId) return;
    try {
        if (!(await ensureAuth())) return;
        await sbFetch(`/rest/v1/gear_scenes?id=eq.${encodeURIComponent(sc.shareId)}`, {
            method: 'PATCH',
            headers: { Prefer: 'return=minimal' },
            body: JSON.stringify({ name: sc.name, data: sceneForShare(sc) }),   // 만료는 안 건드림
        });
    } catch (e) { console.warn('공유 링크 자동 반영 실패 — 다음 저장 때 재시도', e); }
}
...
document.addEventListener('keydown', e => {
    // 체크리스트가 열려 있으면 ESC 로 닫는다 (다른 단축키보다 우선)
    const clv = document.getElementById('checklist-view');
    if (clv && clv.style.display !== 'none') {
        if (e.key === 'Escape') { e.preventDefault(); closeChecklist(); }
        return;
    }
...
function openCat(c, init) {
    const app = document.getElementById('app');
    if (!init && !isMobile()) {   // 모바일은 서랍이 대체 — pal-hidden 토글 안 함
...
// ───────── 차량 적재 체크리스트 ─────────
// 부품 블록이 어느 슬롯(렌즈·배터리…)에 붙었는지 이름을 찾는다.
function checkSlotName(b, scene) {
    if (!b.parent) return '';
    const parent = scene.blocks[b.parent];
    if (!parent) return '';
    const peq = EQUIPMENT.find(e => e.id === parent.eqId);
    const slots = (peq && SLOTS[peq.cat]) || [];
    const s = slots.find(x => x.k === b.slot);
    return s ? s.n : '';
}
// 현재 씬의 모든 블록(루트+부품)을 카테고리별로 묶는다.
function checklistItems() {
    ...
    for (const [bid, b] of Object.entries(blocks)) {
        ...
        (groups[cat] = groups[cat] || []).push({
            bid, eqId: b.eqId, id: b.eqId,
            name: dispName(eq),
            isPart: !!b.parent,
            slotName: checkSlotName(b, scene),
        });
    }
    const cats = CAT_ORDER.filter(c => groups[c])
        .concat(Object.keys(groups).filter(c => !CAT_ORDER.includes(c)));
    return cats.map(cat => {
        // 루트를 먼저, 그 다음 부품. 같은 종류끼리는 이름순.
        groups[cat].sort((a, b) =>
            (a.isPart - b.isPart) || a.name.localeCompare(b.name, 'ko'));
        return { cat, catName: CAT_NAMES[cat] || cat, icon: CAT_ICONS[cat] || '📦', items: groups[cat] };
    });
}
// 실은 개수 / 전체 개수 (현재 씬 기준)
function checklistProgress() {
    const scene = currentScene();
    const checked = scene.checked || {};
    let total = 0, done = 0;
    for (const bid of Object.keys(scene.blocks || {})) { total++; if (checked[bid]) done++; }
    return { total, done };
}
function renderChecklist() {
    ...
    const scene = currentScene();
    const checked = scene.checked || {};
    const onlyEl = document.getElementById('cl-only');
    const onlyLeft = !!(onlyEl && onlyEl.checked);
    document.getElementById('cl-scene').textContent = scene.name || '차량 체크리스트';
    const { total, done } = checklistProgress();
    document.getElementById('cl-prog').innerHTML = total
        ? `실음 <b>${done}</b> / ${total}` + (done === total ? ' · ✅ 전부 실었습니다' : '')
        : '이 씬에 장비가 없습니다';
    document.getElementById('cl-progfill').style.width = (total ? Math.round(done / total * 100) : 0) + '%';
    const body = document.getElementById('cl-body');
    if (!total) {
        body.innerHTML = '<div class="cl-empty">이 씬(배치도)에 배치된 장비가 없습니다.<br>' +
            '배치도에서 장비를 놓으면 여기 체크리스트에 나타납니다.</div>';
        return;
    }
    let h = '', shown = 0;
    for (const g of checklistItems()) {
        const items = onlyLeft ? g.items.filter(it => !checked[it.bid]) : g.items;
        if (!items.length) continue;
        h += `<div class="cl-cat"><div class="cl-cat-h">${g.icon} ${esc(g.catName)} ` +
             `<span class="cl-cat-n">${g.items.length}</span></div>`;
        for (const it of items) {
            shown++;
            const on = !!checked[it.bid];
            const tag = it.slotName ? `<span class="cl-part-tag">${esc(it.slotName)}</span>` : '';
            h += `<div class="cl-row ${on ? 'done' : ''}" onclick="toggleCheck('${it.bid}')">
                    <div class="cl-box">${on ? '✓' : ''}</div>
                    <div class="cl-tx"><div class="cl-name">${esc(it.name)}${tag}</div>
                      <div class="cl-id">${esc(it.id)}</div></div>
                  </div>`;
        }
        h += `</div>`;
    }
    if (!shown && onlyLeft) h = '<div class="cl-empty">안 실은 장비가 없습니다. 🎉<br>모두 실었어요.</div>';
    body.innerHTML = h;
}
function openChecklist() {
    const scene = currentScene();
    if (!scene.checked) scene.checked = {};
    const view = document.getElementById('checklist-view');
...
function toggleCheck(bid) {
    const scene = currentScene();
    if (!scene.blocks || !scene.blocks[bid]) return;   // 사라진 블록이면 무시
    if (!scene.checked) scene.checked = {};
    if (scene.checked[bid]) delete scene.checked[bid];
    else scene.checked[bid] = true;
    saveState();          // 로그인 시 서버 동기화까지 자동
    renderChecklist();
}
function resetChecklist() {
    const scene = currentScene();
    if (!scene.checked || !Object.keys(scene.checked).length) return;
    if (!confirm('체크 표시를 모두 해제할까요?')) return;
    scene.checked = {};
    saveState();
    renderChecklist();
}
...
function renderScenePane() {
    ...
    h += `<div class="pane-btns">
          ...
          <div class="pane-sec">차량 적재</div>
          <div class="slot-help" style="margin-bottom:8px">
            이 씬의 장비를 하나씩 체크하며 실을 수 있어요.<br>아이패드로 앱을 열어 확인하기 좋습니다.
          </div>
          <div class="pane-btns">
            <button class="primary" onclick="openChecklist()">✅ 차량 체크리스트</button>
          </div>
          <div class="pane-sec">외부 공유</div>
          <div class="slot-help" style="margin-bottom:8px">
            씬마다 링크는 <b>하나</b>입니다. 배치를 수정하면 링크에 <b>자동 반영</b>돼요.<br>회수는 로그인 후 언제든 가능합니다.
          </div>
          <div class="pane-btns">
            <button class="primary" onclick="shareCurrentScene()">🔗 이 씬 공유하기</button>
          </div>
          ${(() => {
            const shared = Object.entries(state.scenes).filter(([, s]) => s.shareId);
            return shared.length ? `<div class="pane-sec">공유 중인 씬</div>` +
              shared.map(([, s]) => `
              <div class="share-row">
                <span class="sr-n">${esc(s.name)}</span>
                <span class="sr-d">${s.shareAt ? fmtShareTime(s.shareAt) : ''}${s.shareDays ? ` · ${s.shareDays}일` : ''}</span>
                <button style="padding:3px 8px;font-size:10px"
                    onclick="copyShareUrl('${s.shareId}')">복사</button>
                <button class="danger" style="padding:3px 8px;font-size:10px"
                    onclick="revokeShare('${s.shareId}')">회수</button>
              </div>`).join('') : '';
          })()}
...
// ── 평면도 두 손가락 핀치 줌/팬 (모바일) ──
// 이전 프레임 중점 아래의 세계점을 현재 중점에 고정 → 줌과 팬이 자연스럽게 동시에 됨.
let floorPts = new Map();
let floorPinch = null;      // {dist0, zoom0, mx, my}
let floorPinching = false;
function floorPinchMove() {
    if (!floorPinch || floorPts.size < 2) return;
    const wrap = document.getElementById('floor-wrap');
    if (!wrap) return;
    const f = F();
    const pi = pinchInfo([...floorPts.values()]);
    const wr = wrap.getBoundingClientRect();
    const z0 = f.zoom;
    // 이전 중점 아래의 세계점(현재 줌 기준)
    const Wx = (wrap.scrollLeft + (floorPinch.mx - wr.left)) / z0;
    const Wy = (wrap.scrollTop  + (floorPinch.my - wr.top))  / z0;
    const z1 = Math.max(12, Math.min(160, Math.round(floorPinch.zoom0 * pi.dist / floorPinch.dist0)));
    if (z1 !== z0) { f.zoom = z1; renderFloor(); }
    // 그 세계점이 현재 중점 아래 오도록 스크롤(줌+팬 동시)
    wrap.scrollLeft = Wx * z1 - (pi.mx - wr.left);
    wrap.scrollTop  = Wy * z1 - (pi.my - wr.top);
    floorPinch.mx = pi.mx; floorPinch.my = pi.my;
}
function attachFloorPinch(wrap) {
    wrap.addEventListener('pointerdown', e => {
        floorPts.set(e.pointerId, { x: e.clientX, y: e.clientY });
        if (floorPts.size === 2) {                 // 두 번째 손가락 = 핀치 시작
            floorPinching = true;
            if (fMq) endFloorMarquee();
            if (fPan) endFloorPan();
            const pi = pinchInfo([...floorPts.values()]);
            floorPinch = { dist0: pi.dist, zoom0: F().zoom, mx: pi.mx, my: pi.my };
        }
    });
    wrap.addEventListener('pointermove', e => {
        if (!floorPinching) return;
        if (floorPts.has(e.pointerId)) floorPts.set(e.pointerId, { x: e.clientX, y: e.clientY });
        if (floorPts.size >= 2) { e.preventDefault(); floorPinchMove(); }
    });
    const up = e => {
        floorPts.delete(e.pointerId);
        if (floorPts.size < 2) { floorPinch = null; }
        if (floorPts.size === 0) { if (floorPinching) saveState(); floorPinching = false; }
    };
...
(function initFloorEvents() {
    ...
    attachFloorPinch(wrap);        // 두 손가락 핀치 줌 (모바일)
...
// 두 손가락(포인터)의 거리와 중점 — 3D·평면도 핀치 공용
function pinchInfo(list) {
...
function attachOrbit(canvas) {
    ...
    const pts = new Map();     // 활성 포인터(터치) 추적
    let pinch = null;          // 두 손가락 핀치 상태 {dist,mx,my}
    canvas.addEventListener('contextmenu', e => e.preventDefault());
    canvas.addEventListener('pointerdown', e => {
        pts.set(e.pointerId, { x: e.clientX, y: e.clientY });
        if (pts.size >= 2) {                     // 두 번째 손가락 = 핀치 시작
            mode = null;
            if (gizDrag) endGizDrag();
            if (freeDrag) endFreeDrag();
            pinch = pinchInfo([...pts.values()]);
    ...
    canvas.addEventListener('pointermove', e => {
        ...
        if (pinch && pts.size >= 2) {            // 핀치: 벌리면 줌인, 중점 이동만큼 팬
            const pi = pinchInfo([...pts.values()]);
            const o = R3.orbit;
            if (pi.dist > 0 && pinch.dist > 0)
                o.dist = Math.max(1.2, Math.min(60, o.dist * pinch.dist / pi.dist));
            const k = o.dist * 0.0018, b = viewBasis(o.theta);
            const dmx = pi.mx - pinch.mx, dmy = pi.my - pinch.my;
            o.tx -= (b.rx * dmx + b.fx * dmy) * k;
            o.tz -= (b.rz * dmx + b.fz * dmy) * k;
            pinch = pi;
            return;
        }
    ...
    const stop = (e) => {
        ...
        if (pts.size === 0) { mode = null; canvas.style.cursor = ''; }
    ...
    if (eq.cat === 'TRP' && isTerisTripod(eq)) {  // ── 전용 삼각대 (Teris TSN6CF-Q) ──
        ...
        if (supId) {
            ...
            if (isGim && typeof djiRs4Pro === 'function') {   // DJI RS4 Pro 짐벌 — plate 상면을 카메라 높이에 맞춤
                ...
                if (isTerisTripod(supEq)) {              // 전용 삼각대 (Teris) — 헤드 위에 장비 얹음
                    grp.add(terisTsn6cfTripod(Object.assign({}, ssp, { hBowl: standH })));
                } else if (isCStandPro(supEq)) {         // 전용 C스탠드 (PRO-40T)
                    grp.add(valensPro40t(Object.assign({}, ssp, {
                        h: Math.max(ssp.hMin || 0.5, standH) })));
                    if (armKitOf(it))
                        gripArm(grp, ssp.arm || 1.0, standH - 0.02, yaw,
                            rigParts(it).filter(p => p.slot === 'hang'));
                } else if (isAStandPro(supEq)) {         // 전용 A스탠드 (PRO-403A) — A·작은A·T
                    grp.add(valensPro403a(Object.assign({}, ssp, {
                        h: Math.max(ssp.hMin || 0.5, standH) })));
                } else {                                 // 일반 폴백 (Manfrotto 삼각대 등)
                    legSet(grp, ssp.w, standH * (isTrp ? 0.84 : 0.3));
